...
if __name__ == '__main__': 
  try:
    ...
    #wheelbase = rospy.get_param('~wheelbase', 0.143) #foe mini_akm
    #wheelbase = rospy.get_param('~wheelbase', 0.320) #foe senior_akm
    #wheelbase = rospy.get_param('~wheelbase', 0.503) #foe top_akm_bs
    wheelbase = rospy.get_param('~wheelbase', 0.549) #foe top_akm_dl
    ...
    rospy.loginfo("Node 'cmd_vel_to_ackermann_drive' started.\nListening to %s, publishing to %s. Frame id: %s, wheelbase: %f", "smoother_cmd_vel", ackermann_cmd_topic, frame_id, wheelbase)
